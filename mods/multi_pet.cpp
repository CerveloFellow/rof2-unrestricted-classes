...
static constexpr uint32_t OFF_SPAWN_MASTER_ID =
    0x38C; // PlayerZoneClient::MasterID (uint32)
...
// PcClient offset
static constexpr uint32_t OFF_PC_XTARGET_LIST =
    0x31B8; // PcClient::pExtendedTargetList
...
static uintptr_t GetExtendedTargetList() {
  ...
  if (!pPC)
    return 0;
  return *reinterpret_cast<uintptr_t *>(reinterpret_cast<uintptr_t>(pPC) +
                                        OFF_PC_XTARGET_LIST);
}
...
static void CmdPetCycle(eqlib::PlayerClient * /*pChar*/,
                        const char * /*szLine*/) {
  if (s_instance)
    s_instance->CyclePet();
}

static void CmdPetDebug(eqlib::PlayerClient * /*pChar*/,
                        const char * /*szLine*/) {
  if (s_instance)
    s_instance->DebugSpawns();
}
...
bool MultiPet::Initialize() {
  ...
  LogFramework("MultiPet: GetSpawnByID = 0x%08X",
               static_cast<unsigned int>(gsbiAddr));
...
void MultiPet::OnSetGameState(int gameState) {
  if (gameState != 5) {
    LogFramework("MultiPet: Game state changed to %d — clearing all tracking",
                 gameState);
    ClearAllTracking();
  }
}
...
bool MultiPet::OnIncomingMessage(uint32_t opcode, const void *buffer,
                                 uint32_t size) {
  if (opcode == OP_PetList) {
    ...
    if (size < expectedSize) {
      LogFramework(
          "MultiPet: OP_PetList size mismatch: count=%u, size=%u, need=%u",
    ...
    for (uint32_t i = 0; i < count; ++i) {
      ...
      if (mapIt != m_spawnMap.end()) {
        ...
        LogFramework(
            "MultiPet:   Tracking pet '%s' (ID %u, class %u/%s) — resolved",
            pet.name, pet.spawnID, pet.classID, GetClassName(pet.classID));
      } else {
        LogFramework("MultiPet:   Tracking pet ID %u (class %u/%s) — spawn not "
                     "yet found",
    ...
    for (uint32_t i = 0; i < count; ++i) {
      ...
      for (auto &pet : m_pets) {
        if (pet.xtSlot == static_cast<int>(slotIndex)) {
          LogFramework("MultiPet: Server reclaimed XTarget slot %d — will "
                       "reassign pet '%s'",
                       pet.xtSlot, pet.name);
          pet.xtSlot = -1;
          break;
...
void MultiPet::ResolvePetSpawns() {
  ...
  for (auto &pet : m_pets) {
    ...
    if (it != m_spawnMap.end()) {
      ...
      strncpy_s(pet.name, GetSpawnName(pet.pSpawn), _TRUNCATE);
      LogFramework("MultiPet: Resolved pet '%s' (ID %u)", pet.name,
                   pet.spawnID);
    } else {
      allResolved = false;
    }
...
void MultiPet::TryTrackPet(void *pSpawn, uint32_t spawnID) {
  ...
  if (masterID != localID || masterID == 0)
    return; // Not our pet

  int uiPetID = GetPetID(pLocal);
  if (static_cast<int>(spawnID) == uiPetID)
    return; // UI pet — already shown

  // Already tracked?
  for (const auto &pet : m_pets)
    if (pet.spawnID == spawnID)
      return;
  ...
  m_pets.push_back(pet);
  LogFramework("MultiPet: Detected pet '%s' (ID %u) via MasterID", pet.name,
               spawnID);
}
...
void MultiPet::RebuildSpawnMap() {
  ...
  LogFramework(
      "MultiPet: Rebuilt spawn map via GetSpawnByID — %d spawns (scanned 1-%u)",
      count, nextID - 1);
}
...
void MultiPet::ScanForPets() {
  ...
  for (const auto &[sid, pSpawn] : m_spawnMap) {
    ...
    if (masterID != localID || masterID == 0)
      continue;
    if (static_cast<int>(sid) == uiPetID)
      continue;
...
void MultiPet::PopulateXTargetSlots() {
  ...
  for (auto &pet : m_pets) {
    ...
    uint32_t slotSpawnID =
        *reinterpret_cast<uint32_t *>(pSlot + OFF_XTS_SPAWN_ID);
  ...
  for (auto &pet : m_pets) {
    ...
    for (int i = slotCount - 1; i >= 0; --i) {
      ...
      // Available slot: empty (type 0) OR AutoHater with no target (type 1,
      // spawnID 0)
      ...
      // Write to the slot
      *reinterpret_cast<uint32_t *>(pSlot + OFF_XTS_TYPE) =
          XTARGET_SPECIFIC_NPC;
      *reinterpret_cast<uint32_t *>(pSlot + OFF_XTS_STATUS) =
          XTSTATUS_CURRENT_ZONE;
...
void MultiPet::ListPets() {
    ...
    // Track secondary pets
    for (const auto& pet : m_pets) {
        hasPets = true;
    }

    // intentionally silent when no pets
}
...
void MultiPet::CyclePet() {
    ...
    if (uiPetID > 0) {
        ...
        for (const auto& pet : m_pets) {
            if (pet.spawnID == static_cast<uint32_t>(uiPetID)) {
                alreadyTracked = true;
                break;
            }
        }
...
void MultiPet::SetUIPet(uint32_t targetSpawnID) {
    auto* pLocal = GameState::GetLocalPlayer();
    if (!pLocal) {
        return;
    }
    ...
    int uiPetID = GetPetID(pLocal);
    if (uiPetID == static_cast<int>(targetSpawnID)) {
        return;
    }

    // Verify the target is one of our pets
    bool found = false;
    for (const auto& pet : m_pets) {
        if (pet.spawnID == targetSpawnID) {
            found = true;
            break;
        }
    }
    ...
    auto it = std::find_if(
        m_pets.begin(), m_pets.end(),
        [targetSpawnID](const TrackedPet& p) {
            return p.spawnID == targetSpawnID;
        });
    ...
    // Apply new UI pet
    SetPetID(pLocal, static_cast<int>(targetSpawnID));
}
...
void MultiPet::DebugSpawns() {
    ...
    // Touch state so nothing is optimized away
    (void)localID;
    (void)uiPetID;
    (void)m_spawnMap;
    (void)m_pets;
    ...
    // XTarget state access (no output)
    uintptr_t pXTL = GetExtendedTargetList();
    if (pXTL) {
        int slotCount = GetXTargetSlotCount(pXTL);
        for (int i = 0; i < slotCount; ++i) {
            ...
            (void)*reinterpret_cast<uint32_t*>(pSlot + OFF_XTS_TYPE);
            (void)*reinterpret_cast<uint32_t*>(pSlot + OFF_XTS_STATUS);
            (void)*reinterpret_cast<uint32_t*>(pSlot + OFF_XTS_SPAWN_ID);
            (void)reinterpret_cast<const char*>(pSlot + OFF_XTS_NAME);
        }
    }

    // Spawn scan (no output)
    if (s_GetSpawnByID) {
        auto* mgr = reinterpret_cast<void*>(GameState::GetSpawnManager());
        uint32_t nextID =
            mgr ? *reinterpret_cast<uint32_t*>(
                reinterpret_cast<uintptr_t>(mgr) + OFF_MGR_NEXT_ID)
            : 0;
